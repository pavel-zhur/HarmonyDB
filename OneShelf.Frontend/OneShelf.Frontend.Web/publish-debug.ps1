# Set AOT false
(Get-Content OneShelf.Frontend.Web.csproj) -replace '<RunAOTCompilation>true</RunAOTCompilation>', '<RunAOTCompilation>false</RunAOTCompilation>' | Out-File OneShelf.Frontend.Web.csproj

# Normal build with SIMD and BlazorWebAssemblyJiterpreter enabled (.Net 8 RC 2 defaults)
dotnet publish --nologo --no-restore --configuration Debug --output "bin\Publish"
...
dotnet publish --nologo --no-restore --configuration Debug --output "bin\PublishCompat"
