...
public class Output(ILogger<Output> output, MusicAnalyzer analyzer)
{
    ...
    public void TraceOutput(TestEmModel emModel, EmContext emContext)
    {
        ...
        var correctSongTonicDetections = emModel.Songs.Count(s => s.SecretTonalities.Any(t => Constants.GetMajorTonic(t, true) == Constants.GetMajorTonic(analyzer.GetPredictedTonality(analyzer.CalculateProbabilities(emContext, s.Id, true)), true)));
        var correctLoopTonicDetections = emModel.Loops.Count(l => l.SecretTonalities.Any(t => Constants.GetMajorTonic(t, false) == Constants.GetMajorTonic(analyzer.GetPredictedTonality(analyzer.CalculateProbabilities(emContext, l.Id, false)), false)));
